...
Send out data to USB port and also on UART, eg.  for Serial-2-Net device
...
MFREQ = 250000000  # CPU frequency in Hz (typ. 125 MHz or 250 MHz)
...
class pulsedelay:
    ...
    """
    def get(self):
        self.sm.init(PULSE_LOW_DELTA,freq=MFREQ,
                     in_base=(self.pin1),
                     jmp_pin=(self.pin2))
...
def main():
    ...
    m.freq(MFREQ) # can overclock to 250 MHz. Wheee!    

    # nominal timings from 1-PPS, 10% duty pulse
    #nomInt = (12500000, 125000000, 137500000, 250000000)
    
    uart = machine.UART(0, 115200)         # use this UART (Pico has two)
    nomInt = (0, 0, 0, 0)

    outs = "Pico 4-Interval PIO timer + UART out v0.5  18-March-2021 J.Beale\n"
    uart.write(outs)
    print(outs,end='')
    ...
    while True:
        ...
        outs = ""
        for n in newVals:
            delta = (n+i) - nomInt[i] # +i correction due to mov,push overhead
            outs += ("%d" % delta)
            if i < 3:
                i += 1
                outs += ", "
        ...
        lcount += 1
        if (lcount >= 10):
          uart.write("**HALT") # send end this packet
          print("**HALT sent")
          lcount = 0
