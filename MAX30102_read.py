...
def wdat(addr,data):  
  ...
  sleep_ms(20)  # chip locks up without some delay here
...
wdat(SPO2Cfg,[7])        # 100 sps, 411 usec pulse width
...
readAvg = 4  # how many readings to average together
warmupCycles = 40  # how many values to run before printing
...
print("Red, IR") # CSV column headers
while True:
  valR = rdat(FIFORPT, 1)
  valW = rdat(FIFOWPT, 1)
  if (valR[0] != valW[0]):  # new data if read & write ptr not equal
    ...
    if (rCnt == readAvg):
      ...
      if (pCnt > warmupCycles):
        print("%d,%d,%d,%d" % (rHP2,iHP2,rLP,iLP))  # display Red and IR readings
      rCnt = 0
